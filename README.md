...
- PostgreSQL 14+ (local, or AWS RDS in production)
...
│
├── deploy/                      AWS ECS Fargate assets (task defs, ECR push script)
├── docker-compose.yml           Local/self-hosted stack (Postgres + backend + frontend)
├── AWS_DEPLOYMENT.md            Full AWS deployment guide (EC2 + Docker, or ECS Fargate)
...
- **Node.js** 18 or later
- **PostgreSQL** 14 or later (local, RDS, or another hosted instance)
...
This app deploys as two Docker images (`backend/Dockerfile`, `frontend/Dockerfile`) plus a
PostgreSQL database. See **[AWS_DEPLOYMENT.md](./AWS_DEPLOYMENT.md)** for the full guide, covering:

- **Docker Compose on a single EC2 instance** — simplest, good for a demo/low-traffic deployment.
- **ECS Fargate** — scalable, no servers to patch (task definition templates and an ECR push
  script are in [`deploy/`](./deploy)).

For local testing of the same Docker setup:
